...
import { listenHealth } from "./health.js";
...
export class Bot {
  ...
  async start(): Promise<void> {
    ...
    if (this.cfg.port && Number.isFinite(this.cfg.port)) {
      this.health = listenHealth(this.cfg.port, () => this.lastPollAt);
    }
    this.schedule(0);
  }
  ...
  async stop(): Promise<void> {
    ...
    if (this.health) {
      await new Promise<void>((resolve) => this.health!.close(() => resolve()));
      this.health = null;
    }
    if (this.store) {
  ...
  private async pollOnce(): Promise<void> {
    ...
    const gen = this.gen;
    try {
      if (this.cfg.disabledEnv || (await this.store.killSwitchOn())) {
      ...
      const filtered = cur.firstBootDone
        ? items
        : skipStaleFirstBoot(items, Date.now(), this.cfg.maxAgeMinutes);
  ...
  private async consume(n: Notification, gen: number): Promise<void> {
    ...
    try {
      ...
      if (this.cfg.disabledEnv || (await this.store.killSwitchOn())) return;

      const blocked = authorBlocked(parsed.author, this.botPk, this.cfg.blocklist);
      if (blocked) {
      ...
      for (const p of chainViews) {
        ...
        chainPosts.push(asChainPost(p, u));
      }
      ...
      let text: string;
      try {
        text = await completeReply(this.cfg, assemblePrompt(this.botPk, mentionPost, chainPosts));
      } catch (e) {
